...
class PyRobot:
    ...
    def pre_market_open(self) -> bool:
        ...
        right_now = datetime.now().repalce(tzinfo=timezone.utc).timestamp()
    ...
    def post_market_open(self) -> bool:
        ...
        market_end_time = (
            datetime.now()
            .replace(hour=20, minute=00, second=00, tzinfo=timezone.utc)
            .timestamp()
        )
        right_now = datetime.now().repalce(tzinfo=timezone.utc).timestamp()

        if market_end_time <= right_now <= post_market_start_time:
            return True
        else:
            return False
